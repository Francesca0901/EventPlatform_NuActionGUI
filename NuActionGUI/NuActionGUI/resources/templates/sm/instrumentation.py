...
def get_context():
    def __securecontext__():
        roles = {
            {% for r in sm.roles -%} 
            '{{- r.name}}': {{projectname}}SecurityModel.Role.{{- r.name}},
            {% endfor -%}
        }
        c = current_user if not current_user.is_anonymous else None
        r = c.role if not c is None else {{projectname}}SecurityModel.Role.{{- sm.anonymousRole}}
        ...
        if check_context(context):
            ...
            if isinstance(ret, list):
                ...
                if self._sm.permit(role, attr, Action.read, resource, caller):
                    ...
                    # role = caller.role if not caller.is_anonymous else {{projectname}}SecurityModel.Role.{{- sm.anonymousRole}}
                    # caller = "User '" + caller.username + "'" if not caller.is_anonymous else "{{- sm.anonymousRole}} user"
                    # role = role.name
                    # resource = resource.__class__.__name__
